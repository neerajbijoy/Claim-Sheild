...
function calculateProcedureRiskPriority(procedure, checks = [], findings = [], historicalSignal = null, payerName = 'Payer') {
  ...
  if (missingDocFinding) {
    riskScore += 45;
    riskFactors.push(`Missing required attachment or evidence (${missingDocFinding.title || 'Diagnostic radiograph'}).`);
  }
  ...
  if (supportFinding) {
    riskScore += 25;
    riskFactors.push(`Clinical justification lacks sufficient structural breakdown or medical necessity terms.`);
  }
  ...
  if (historicalSignal && historicalSignal.historical_signal === 'HIGH') {
    riskScore += 20;
    riskFactors.push(`High historical audit scrutiny: multiple similar ${cdtCode} submissions had documentation deficiencies.`);
  ...
  if (priority === 'HIGH') {
    explanation = riskFactors.length > 0
      ? `Action Recommended: ${riskFactors[0]}`
      : `Action Recommended: ${cdtCode} has missing or deficient documentation items prior to submission.`;
  } else if (priority === 'MEDIUM') {
    explanation = riskFactors.length > 0
      ? `Review Recommended: ${riskFactors[0]}`
      : `Review Recommended: clinical documentation or narrative can be strengthened.`;
  } else {
    explanation = `Criteria Satisfied: Procedure ${cdtCode} (Tooth #${tooth}) satisfies all documentation and payer requirements.`;
  }
